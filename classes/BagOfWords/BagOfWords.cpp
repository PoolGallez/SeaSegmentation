...
BagOfWords::BagOfWords(FeatureExtractor extractor, cv::TermCriteria termCriteria) : bowDE(extractor.getExtractor(), extractor.getMatcher())
{
...
void BagOfWords::computeHistograms(cv::Mat &histograms, cv::Mat &labels, std::vector<cv::Mat> positives, std::vector<cv::Mat> negatives)
{
    ...
    for (cv::Mat positive : positives)
    {
        ...
        labelsMat.push_back(cv::Mat(1, 1, CV_32SC1, WATER_CLASS));
        i++;
    }
    positives.clear();
    ...
    for (cv::Mat negative : negatives)
    {
        ...
        histograms.push_back(histogram);
        labelsMat.push_back(cv::Mat(1, 1, CV_32SC1, NON_WATER_CLASS));
    }
    negatives.clear();
    labels = labelsMat;
